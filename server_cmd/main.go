...
// buildThumbsJSONPayload enumerates all thumbnails under dir/subnails and returns
// a JSON document like: {"photos":[{"id":"xxx","data":"<base64>","media":"jpg"}, ...]}
func buildThumbsJSONPayload(dir string) ([]byte, error) {
	...
	// Collect and sort file names for stable ordering
	var names []string
	for _, e := range entries {
		if e.IsDir() {
			continue
	...
	out := payload{Photos: make([]photoItem, 0, len(names))}
